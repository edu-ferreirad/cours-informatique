...
const STEPS = [
  ...
  },
  {
    ...
    gps: { lat: 46.2011, lng: 6.1462, label: "7, Rue de la Corraterie (tour de l'Escalade, près du Grand Théâtre)" },
  ...
  },
  {
    ...
    transition: "Mais la Mère Royaume n'est pas la seule à avoir lancé quelque chose ce soir-là. Notre seconde héroïne, Dame Piaget, fit de même avec un autre objet, depuis sa fenêtre, pour aider les défenseurs à ouvrir un passage secret dans la muraille.",
    brief: "Charade : je rentre toujours en premier, et je sors toujours en dernier. Qui suis-je ? Le passage qu'elle a permis d'ouvrir existe encore aujourd'hui : le Passage de Monetier, fermé au public toute l'année sauf le week-end de l'Escalade, mais dont la grille — frappée des armoiries de Genève — reste visible depuis la rue.",
    gps: { lat: 46.2015, lng: 6.1484, label: "Rue du Perron 19 (grille du Passage de Monetier — ⚠ à vérifier sur place)" },
    question: "Je rentre toujours en premier, et je sors toujours en dernier : quel est cet objet ? Prenez en photo la grille du Passage de Monetier, rue du Perron, pour valider votre passage.",
    ...
    cipherTool: "romaine (I=A, II=B…)",
    funFact: "Le passage lui-même ne s'ouvre qu'une fois par an, le week-end de l'Escalade — le reste du temps, seule sa grille armoriée est visible. Une carte de son tracé à travers la Vieille Ville est gravée sur le mur, juste à l'entrée.",
    bonus: { question: "Bonus (+5 pts) : à quelle autre rue débouche l'autre extrémité du Passage de Monetier ?", answer: "BARRIERES", altAnswers: ["RUE DES BARRIERES"] }
  },
  {
    id: 6,
    type: "text",
...
function initWelcomeScreen() {
  ...
  if (startLabel && STEPS[0] && STEPS[0].gps) {
    ...
    startLabel.innerHTML = `Rendez-vous, avant même d'ouvrir le jeu, à : <strong>${startGps.label}</strong>. ` +
...
function renderTextStep(step, container) {
  ...
  if (step.photoRequired) {
    ...
    extraInput.addEventListener("change", () => {
      ...
      extraUserImg.src = url; extraUserImg.classList.remove("hidden"); extraPlaceholder.classList.add("hidden");
      ...
      if (!("geolocation" in navigator) || !step.gps) { finishPhoto("Position non vérifiable sur cet appareil — photo acceptée.", "warn"); return; }
