...
async function run() {
  ...
  for (let round = 0; round < ALL_STEPS.length + 2; round++) {
    ...
    if (type === 'text') {
      ...
      await flushTimers(15);
      const contBtn1 = activeStep.querySelector('.continue-to-next-btn');
      if (contBtn1) contBtn1.click(); else if (answer) errors.push(`Pas de bouton "Continuer" trouvé après réponse correcte, secteur ${numText}`);
    } else if (type === 'photo') {
      ...
      const contBtn2 = activeStep.querySelector('.continue-to-next-btn');
      // Vérifie spécifiquement le mini-jeu du canon (secteur 7) : doit
      // être encore présent et cliquable APRÈS la validation, avant
      // qu'on continue — c'était exactement le bug signalé.
      const cannonBtn = activeStep.querySelector('.cannon-fire-btn');
      if (cannonBtn) {
        console.log('  mini-jeu canon présent, clic de test...');
        cannonBtn.click();
        await flushTimers(15);
        console.log('  mini-jeu canon toujours dans le DOM après clic :', !!document.querySelector('.cannon-block'));
      }
      if (contBtn2) contBtn2.click(); else errors.push(`Pas de bouton "Continuer" trouvé après photo validée, secteur ${numText}`);
    } else if (type === 'puzzle') {
      ...
      await flushTimers(15);
      const contBtn3 = document.querySelector('.continue-to-next-btn');
      if (contBtn3) contBtn3.click(); else if (solved) errors.push(`Pas de bouton "Continuer" trouvé après puzzle résolu, secteur ${numText}`);
    }
