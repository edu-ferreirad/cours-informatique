...
const ArXR = (() => {
  ...
  // ---------------------------------------------------------------
  // SESSION UNIQUE POUR TOUTE LA PARTIE.
  // Avant : chaque chapitre ouvrait une NOUVELLE session WebXR
  // (session.end() puis requestSession() à nouveau). Or le repère
  // "local-floor" d'une session WebXR est réinitialisé à CHAQUE
  // nouvelle session, à l'endroit où se trouve le téléphone au moment
  // où elle démarre — donc les coordonnées enregistrées à la
  // calibration ne voulaient plus rien dire dès la session suivante :
  // les objets semblaient "se déplacer selon où on est dans la classe".
  // Fix : on garde LA MÊME session (et le même renderer/scene/canvas)
  // du premier "Commencer la calibration" jusqu'à la fin de la partie.
  // Entre deux chapitres, on ne fait que vider/remplir la scène — on
  // ne referme jamais la session. Un seul listener "select" est posé,
  // une fois, et délègue à `activeHandlers` (mis à jour à chaque
  // chapitre) au lieu d'empiler un nouveau listener à chaque fois.
  // ---------------------------------------------------------------
  let selectListenerBound = false;
  let activeHandlers = null; // { onCorrect, onWrong, onHint }
  let resolved = false;
  ...
  function initRenderer(canvas) {
    // Une seule fois pour toute la partie : recréer le renderer sur le même
    // canvas alors que la session XR reste ouverte casserait le lien entre
    // les deux (le nouveau renderer n'aurait jamais reçu `setSession`).
    if (renderer) return;
  ...
  async function openSession(overlayRoot) {
    if (session) return session; // session déjà ouverte pour toute la partie : on la réutilise
    ...
    session.addEventListener("end", () => {
      session = null;
      selectListenerBound = false;
      calib = null; // la session est vraiment finie : une recalibration sera nécessaire
      // Le renderer était lié à CETTE session/ce canvas : s'il en faut un
      // nouveau (recalibration sur un nouveau canvas), il doit être recréé
      // depuis zéro — sinon initRenderer() le sauterait par erreur.
      if (renderer) { renderer.setAnimationLoop(null); renderer.dispose(); }
      renderer = null; scene = null; camera = null; raycaster = null;
      const cb = sessionEndCallback;
  ...
  // Un seul listener "select" pour toute la partie, qui délègue au
  // handler du chapitre EN COURS (activeHandlers) au lieu d'en empiler
  // un par chapitre (ce qui ferait réagir plusieurs anciens chapitres
  // au même tap).
  function bindSelectListener() {
    if (selectListenerBound) return;
    selectListenerBound = true;
    session.addEventListener("select", (event) => {
      if (!activeHandlers || resolved) return;
      let origin, dir;
      ...
      if (anchor.role === "hint") {
        flashHint(visual);
        if (activeHandlers.onHint) activeHandlers.onHint(anchor);
        return;
      }
      if (anchor.correct) {
        resolved = true;
        if (activeHandlers.onCorrect) activeHandlers.onCorrect(anchor);
      } else {
        flashWrong(visual);
        if (activeHandlers.onWrong) activeHandlers.onWrong(anchor);
      }
    });
  }
  ...
  async function calibrate({ canvas, overlayRoot, onCalibrated, onEnd }) {
    initRenderer(canvas);
    await openSession(overlayRoot); // pose aussi le listener "select" générique + le listener "end"

    let done = false;
    // Listener dédié à LA calibration uniquement (une seule fois, en plus du
    // listener générique posé par openSession — celui-ci ignore le tap tant
    // qu'aucun chapitre n'a défini activeHandlers, donc pas de conflit).
    session.addEventListener("select", function calibrationTap() {
      if (done) return;
      done = true;
    ...
    // onEnd n'est appelé que si la session se termine VRAIMENT (l'élève
    // quitte la RA, ou une erreur) — plus jamais après un simple chapitre,
    // puisqu'on ne referme plus la session entre les chapitres.
    if (onEnd) registerSessionEndCallback(onEnd);
  ...
  // Les appelants (calibration, exploration de chapitre) veulent parfois être
  // notifiés si la session se termine complètement (pas juste "chapitre
  // fini"). Un seul "gardien" à la fois suffit — le chapitre en cours
  // remplace celui du précédent au lieu de s'empiler indéfiniment (sinon,
  // au bout de 8 chapitres, une vraie coupure réveillerait 8 callbacks
  // obsolètes en même temps).
  let sessionEndCallback = null;
  function registerSessionEndCallback(cb) { sessionEndCallback = cb; }
  ...
  async function exploreChapter({ canvas, overlayRoot, anchors, onCorrect, onWrong, onHint, onEnd }) {
    ...
    initRenderer(canvas);
    await openSession(overlayRoot); // no-op si déjà ouverte : MÊME session, MÊME origine que la calibration

    clearAnchors(); // retire les panneaux du chapitre précédent avant de placer les nouveaux

    resolved = false;
    anchorMeshes = anchors.flatMap((rawA, idx) => {
      ...
      // Zone de visée agrandie et INVISIBLE (quasi transparente, mais
      // "visible=true" pour que le raycaster la teste), même position et
      // orientation, mais bien plus grande que le panneau réel : avant ce
      // correctif, il fallait toucher pile la surface fine du panneau à
      // 4-7 m, d'où les nombreux taps qui ne "prenaient" pas.
      const hitGeo = new THREE.PlaneGeometry(1.5, 1.15);
      ...
      hitPlane.userData.visualMesh = panel; // pour flasher le VRAI panneau, pas la zone invisible
    ...
    // délègue au listener "select" unique posé par openSession()
    activeHandlers = { onCorrect, onWrong, onHint };

    if (onEnd) registerSessionEndCallback(onEnd);

    // le rendu tourne déjà en continu depuis la calibration ; on s'assure
    // simplement qu'il est bien lancé si jamais on entre ici en premier.
    renderer.setAnimationLoop((_, frame) => {
      if (frame) latestFrame = frame;
      renderer.render(scene, camera);
    });
  ...
  // Appelé quand l'élève quitte volontairement la porte RA d'un chapitre
  // (bouton "Quitter"/"Continuer") SANS mettre fin à la session globale —
  // on vide juste la scène et on retire les handlers de ce chapitre.
  function leaveChapter() {
    clearAnchors();
    activeHandlers = null;
    sessionEndCallback = null; // plus aucune porte RA active : personne à notifier si la session finit
  }
...
function fakeCalibrate() {
  ...
  return { isAvailable, calibrate, isCalibrated, exploreChapter, leaveChapter, endSession, resetCalibration, fakeCalibrate };
})();
