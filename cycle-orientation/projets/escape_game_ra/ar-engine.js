...
const ArXR = (() => {
  ...
  function makePanel(text, { correct, image, emoji, hintRole } = {}) {
    ...
    const geo = new THREE.PlaneGeometry(0.58, 0.375);
  ...
  async function exploreChapter({ canvas, overlayRoot, anchors, onCorrect, onWrong, onHint, onEnd }) {
    ...
    anchorMeshes = anchors.map((rawA, idx) => {
      ...
      const panel = makePanel(a.label, { correct: a.role === "hint" ? undefined : a.correct, hintRole: a.role === "hint", image: preloaded[idx], emoji: rawA.emoji });
