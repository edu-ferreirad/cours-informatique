...
const ArXR = (() => {
  ...
  function makePanel(text, { correct, image } = {}) {
    ...
    if (image) {
      // zone image (haut du panneau), contenue et centrée
      const padding = 22;
      const imgBoxW = 512 - padding * 2;
      const imgBoxH = 190;
      const imgBoxX = padding, imgBoxY = 20;
      ...
      ctx.save();
      roundRect(ctx, imgBoxX, imgBoxY, imgBoxW, imgBoxH, 14);
      ctx.clip();
      ...
      wrapText(ctx, text, 256, 250, 460, 30);
    } else {
  ...
  async function exploreChapter({ canvas, overlayRoot, anchors, onCorrect, onWrong, onEnd }) {
    ...
    anchorMeshes = anchors.map((rawA, idx) => {
      ...
      const panel = makePanel(a.label, { correct: a.correct, image: preloaded[idx] });
      const jitter = a.correct ? 0 : DECOY_JITTER_METERS;
