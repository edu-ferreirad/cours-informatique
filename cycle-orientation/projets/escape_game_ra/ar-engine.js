  ...
  async function exploreChapter({ canvas, overlayRoot, anchors, onCalibrated, onCorrect, onWrong, onHint, onInspect, onEnd }) {
    ...
    function placeAnchors() {
      anchorMeshes = anchors.flatMap((rawA, idx) => {
        ...
        // Zone de visée légèrement agrandie et quasi invisible (mais
        // raycastable) — plus grande que le panneau réel (0,7×0,45) pour
        // ne pas avoir à toucher pile sa fine surface à 4-7 m, mais
        // volontairement MODESTE : certains objets ne sont qu'à ~1 m l'un
        // de l'autre dans la salle (ex. cloche/alarme = 1,40 m) — une zone
        // trop grande les fait se chevaucher, et on sélectionne le voisin
        // au lieu de l'objet visé. 0,85×0,6 reste sûr même dans le pire
        // cas de décalage aléatoire des leurres (DECOY_JITTER_METERS).
        const hitGeo = new THREE.PlaneGeometry(0.85, 0.6);
