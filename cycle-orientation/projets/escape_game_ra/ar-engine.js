...
const ArXR = (() => {
  ...
  // ---------------------------------------------------------------
  // UNE SESSION PAR PORTE RA — ouverte à chaque fois, refermée en
  // sortant (comme à l'origine). Pourquoi revenir en arrière : pendant
  // qu'une session WebXR immersive est active, le téléphone n'affiche
  // QUE la caméra + la zone RA — jamais le reste de la page (le puzzle,
  // les questions...). Une session "permanente" pour toute la partie
  // rendait donc TOUT LE RESTE DU JEU invisible/inutilisable dès la
  // calibration. Le vrai bug d'origine ("les objets bougent selon où
  // je suis dans la classe") ne venait pas du fait de rouvrir une
  // session à chaque porte — mais de réutiliser, dans cette NOUVELLE
  // session, la calibration capturée dans une AUTRE session (chaque
  // session WebXR repart avec sa propre origine de repère "sol").
  // Fix correct : on recalibre (un tap au point de repère) au début de
  // CHAQUE porte RA, dans SA session — pas une session pour tout le jeu.
  // ---------------------------------------------------------------
  ...
  function initRenderer(canvas) {
    // Un nouveau canvas à chaque porte RA (créé/détruit par script.js) :
    // toujours recréer le renderer dessus.
  ...
  function makePanel(text, { correct, image, emoji, hintRole, romanTrap } = {}) {
    ...
    ctx.strokeStyle = hintRole ? "#5fa8e0" : "#e7a94c";
    ctx.lineWidth = 8;
    roundRect(ctx, 8, 8, 664, 424, 28); ctx.stroke();
    ctx.fillStyle = hintRole ? "#a9d4f5" : "#f6cf8a";

    if (romanTrap) {
      // easter egg fidèle à l'original : "dommage" écrit en chiffres romains
      // (1=A, 2=B... comme au dos du cahier ascenseur/escaliers réel).
      // Corrigé : trop petit et trop transparent pour être lu à distance
      // sur un écran de téléphone (15px, opacité 0.55) — agrandi et
      // rendu bien plus contrasté.
      ctx.save();
      ctx.font = "italic 700 22px 'JetBrains Mono', monospace";
      ctx.textAlign = "center";
      ctx.fillStyle = "rgba(255,150,140,0.95)";
      ctx.shadowColor = "rgba(0,0,0,0.8)";
      ctx.shadowBlur = 4;
      ctx.fillText("IV · XV · XIII · XIII · I · VII · V", 340, 412);
      ctx.restore();
    }
  ...
  async function exploreChapter({ canvas, overlayRoot, anchors, onCalibrated, onCorrect, onWrong, onHint, onInspect, onEnd, multiFind, onFound }) {
    ...
    // Si le suivi de la caméra a dérivé (typique en RA sans marqueur, surtout
    // après un temps d'immobilité comme la lecture d'un indice), les objets
    // peuvent sembler avoir "bougé". Rien dans ce code ne les déplace après
    // placeAnchors() — c'est un recalage complet (nouveau point de repère)
    // qui corrige ça, exposé ici pour un bouton "Recalibrer" dans l'UI.
    currentRecalibrate = () => {
      calibDone = false;
      resolved = false;
      inspecting = false;
      anchorMeshes.forEach(m => { if (scene) scene.remove(m); });
      anchorMeshes = [];
    };

    // Zone de visée adaptative : au lieu d'une taille fixe (0,85×0,6) qui
    // devait rester petite PARTOUT à cause des rares objets rapprochés
    // (cloche/alarme ≈ 1,40 m), on calcule pour chaque objet l'écart réel
    // au voisin le plus proche (sur le plan horizontal forward/right) et on
    // en déduit une zone aussi grande que possible SANS jamais chevaucher
    // ce voisin. Résultat : la plupart des objets (souvent 3-6 m d'écart,
    // ex. les écussons/bulletins) deviennent beaucoup plus faciles à viser,
    // et les rares paires rapprochées (cloche/alarme) restent sûres tout en
    // gagnant un peu de marge par rapport à l'ancienne taille fixe.
    const HIT_MIN = 0.6, HIT_MAX = 1.4, HIT_RATIO = 0.68;
    function computeHitWidth(idx, converted) {
      const a = converted[idx];
      if (a.role === "hint") return HIT_MIN;
      let minDist = Infinity;
      converted.forEach((b, j) => {
        if (j === idx || b.role === "hint") return;
        const d = Math.hypot(a.forward - b.forward, a.right - b.right);
        if (d < minDist) minDist = d;
      });
      if (!isFinite(minDist)) minDist = HIT_MAX / 0.7; // objet isolé : zone maximale
      // 0,7× l'écart garde toujours une marge de sécurité (~30%) entre deux
      // zones voisines, même avec le décalage aléatoire des leurres (jitter).
      return Math.min(HIT_MAX, Math.max(HIT_MIN, minDist * 0.7));
    }

    function placeAnchors() {
      const converted = anchors.map(toForwardRight);
      anchorMeshes = converted.flatMap((a, idx) => {
        a.image = preloaded[idx]; // pour le zoom au tap (voir onInspect plus bas)
        a.emoji = anchors[idx].emoji;
        const panel = makePanel(a.label, { correct: a.role === "hint" ? undefined : a.correct, hintRole: a.role === "hint", image: preloaded[idx], emoji: a.emoji, romanTrap: a.romanTrap });
        const jitter = (a.correct || a.role === "hint" || !jitterEnabled) ? 0 : DECOY_JITTER_METERS;
        ...
        const hitW = computeHitWidth(idx, converted);
        const hitGeo = new THREE.PlaneGeometry(hitW, hitW * HIT_RATIO);
        const hitMat = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0.001, side: THREE.DoubleSide, depthWrite: false });
        const hitPlane = new THREE.Mesh(hitGeo, hitMat);
        hitPlane.position.copy(worldPos);
        hitPlane.quaternion.copy(panel.quaternion);
        hitPlane.userData.anchor = a;
        hitPlane.userData.visualMesh = panel; // flasher le VRAI panneau, pas la zone invisible
        panel.userData.visualMesh = panel;
        scene.add(hitPlane);

        return [panel, hitPlane];
      });
    }

    session.addEventListener("select", (event) => {
      // Premier tap de CETTE session = calibration (voir point de repère
      // affiché avant l'appel). Chaque porte RA recalibre dans SA propre
      // session — c'est ce qui évite le décalage d'une session à l'autre.
      if (!calibDone) {
        calibDone = true;
        const pos = new THREE.Vector3();
        camera.getWorldPosition(pos);
      ...
      if (resolved || inspecting) return;
      let origin, dir;
      const inputSource = event.inputSource;
      if (latestFrame && inputSource && inputSource.targetRaySpace) {
        try {
          const refSpace = renderer.xr.getReferenceSpace();
          const pose = latestFrame.getPose(inputSource.targetRaySpace, refSpace);
          if (pose) {
            const m = new THREE.Matrix4().fromArray(pose.transform.matrix);
            origin = new THREE.Vector3().setFromMatrixPosition(m);
            dir = new THREE.Vector3(0, 0, -1).transformDirection(m);
          }
      ...
      const anchor = hitObj.userData.anchor;
      const visual = hitObj.userData.visualMesh || hitObj;
      if (anchor.role === "hint") {
        flashHint(visual);
        if (onHint) onHint(anchor);
        return;
      }
      // Mode "plusieurs trouvailles dans une même session" (ex. les 5
      // icônes des sens de la Salle noire) : chaque objet peut être trouvé
      // indépendamment, la session ne se termine jamais toute seule — seul
      // le bouton "Quitter" y met fin. Pas de zoom/confirmation ici, ces
      // objets ne sont pas des photos à vérifier.
      if (multiFind) {
        if (foundIds.has(anchor.id)) { flashHint(visual); return; }
        foundIds.add(anchor.id);
        flashHint(visual); // flash bleu = trouvaille, pas d'erreur ici
        if (onFound) onFound(anchor);
        return;
      }
      // Un tap = un zoom d'abord, jamais une réponse immédiate — l'objet
      // (surtout un document à lire, ex. le bulletin) reste petit à
      // l'écran à distance normale ; on l'affiche en grand avant de
      // demander confirmation, plutôt que de trancher sur un tap imprécis.
      if (onInspect) {
        inspecting = true;
        onInspect(anchor, (confirmed) => {
          inspecting = false;
          if (!confirmed) return; // "non, ce n'est pas ça" — pas de pénalité, on continue de chercher
          if (anchor.correct) {
            resolved = true;
