...
function openDossier() {
  ...
  function renderTabContent(tab) {
    ...
    if (tab === 5) {
      ...
      if (!lyrics) return `<p class="chapter-text" style="text-align:center; opacity:.7;">Les paroles de « La Belle Escalade » n'ont pas encore été ajoutées.</p>`;
      return `<div class="chapter-text" style="text-align:left; white-space:pre-line;">${escapeHtml(lyrics)}</div>`;
    }
    return `
...
function renderPuzzle(container, ch) {
  ...
  if (p.type === "jigsaw-image") {
    ...
    const widget = el("div", "puzzle-widget");
    widget.style.cssText = "background:#f2ead6; border-radius:10px; padding:20px; text-align:center; color:#2a2318;";
    widget.innerHTML = `
      ...
      <div style="font-family:var(--font-mono); font-size:34px; letter-spacing:14px;">${p.expression}</div>
    `;
    container.appendChild(widget);
    container.appendChild(el("div", "chapter-text", p.prompt));
    renderAnswerForm(container, ch);
