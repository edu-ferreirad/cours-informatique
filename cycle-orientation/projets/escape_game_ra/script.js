...
function openDossier() {
  ...
  const tabs = ["Table numérique", "Chiffres romains", "Roue de César"];
...
function routeToGateOrPuzzle(container, ch, fallbackFn, keyOverride) {
  ...
  if (needsSpatialGate) {
    renderSpatialGate(container, chForGate);
  } else if (needsMinimalArGate) {
    renderMinimalArGate(container, chForGate, fallbackFn);
  } else {
    (fallbackFn || (() => renderPuzzle(container, ch)))();
  }
}
...
function renderSpatialGate(container, ch) {
  ...
  function launch() {
    ...
    ArXR.exploreChapter({
      ...
      onCorrect: (anchor) => {
        ...
        playTone("right");
        launchConfetti();
        $(".xr-overlay-hint", overlayRoot).innerHTML = `<strong style="color:var(--flame-bright); font-size:15px;">✓ Trouvé : « ${anchor.label} » !</strong><br>Touchez « Continuer » ci-dessous pour sortir de la RA.`;
      ...
      onWrong: (anchor) => {
        wrongCount++;
        playTone("wrong");
        dommage.className = "answer-feedback wrong";
        dommage.style.color = "";
      ...
      onHint: () => {
        const costLabel = state.difficulty === "facile" ? "gratuit"
          ...
          : "coûte 1 point";
        const ok = confirm(`Le vieil archiviste peut vous aider (${costLabel}). Voulez-vous un indice ?`);
        if (!ok) return;
        consumeHint();
        dommage.className = "answer-feedback";
        dommage.style.color = "var(--flame-bright)";
        dommage.textContent = `💡 ${ch.hint}`;
      },
      onEnd: () => {
        overlayRoot.classList.add("hidden");
        canvas.remove();
        if (found) {
          // Trouver le bon objet en RA EST la validation — on ne repasse
          // pas par un champ texte (qui accepterait la bonne réponse même
          // si on avait tapé le mauvais objet). On note les points et on
          // avance directement.
          state.score += 3;
          updateScorePill();
          finishChapter(container, ch);
        } else {
          // sortie accidentelle (ou volontaire) avant d'avoir trouvé le bon objet :
          // on ne perd pas la partie, on propose juste de relancer la RA.
  ...
  skipBtn.addEventListener("click", () => renderPuzzle(container, ch));
  ...
  if (p.type === "jigsaw-image") {
    ...
    function onTileClick(slot) {
      ...
      if (order.every((v, i) => v === i)) {
        ...
        setTimeout(() => {
          container.innerHTML = "";
          container.appendChild(el("div", "chapter-text", "<strong>La carte confirme : vous êtes à Genève. Avant d'aller plus loin, un dernier repérage : les outils de décodage doivent être quelque part dans la salle — ils vous serviront toute la partie.</strong>"));
          ...
          routeToGateOrPuzzle(container, toolsCh, () => {
            unlockDossier();
            container.innerHTML = "";
            container.appendChild(el("div", "chapter-text", "<strong>Cherchez l'écusson de la ville dans la salle — méfiez-vous des autres cantons.</strong>"));
            routeToGateOrPuzzle(container, ch, () => finishChapter(container, ch));
          });
        }, 700);
      }
    }
