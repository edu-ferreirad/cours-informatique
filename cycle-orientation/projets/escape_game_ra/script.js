...
$("#btn-calibrate-start").addEventListener("click", async () => {
  ...
  $(".xr-overlay-hint", overlayRoot).textContent = "Visez le sol au point de repère, puis touchez l'écran pour calibrer.";
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = "Commencer la calibration";
    alert("RA indisponible ici — vous pouvez continuer sans.");
  }
});
...
function renderSpatialGate(container, ch) {
  ...
  launchBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      dommage.textContent = "RA indisponible — passage direct à l'énigme.";
      renderPuzzle(container, ch);
    }
  });
...
function renderAnswerForm(container, ch) {
  ...
  const hintLabel = state.jokerUsed ? "Besoin d'un indice ? (–1 pt)" : "Besoin d'un indice ? (1er gratuit 🃏)";
  form.innerHTML = `
  ...
  hintBtn.addEventListener("click", () => {
    ...
    if (!hintUsed) {
      hintUsed = true;
      if (!state.jokerUsed) {
        state.jokerUsed = true; // joker gratuit consommé, une seule fois pour toute l'équipe
      } else {
  ...
  function renderStageDark() {
    ...
    function renderAnswerFormFinal() {
      ...
      hintBtn.addEventListener("click", () => {
        ...
        if (!hintUsed) {
          hintUsed = true;
          if (!state.jokerUsed) { state.jokerUsed = true; }
          else { state.score = Math.max(0, state.score - 1); updateScorePill(); }
        }
      });
      form.addEventListener("submit", (e) => {
        e.preventDefault();
