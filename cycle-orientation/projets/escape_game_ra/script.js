...
const state = {
  ...
  calibrated: false,
  xrCanvas: null,      // canvas RA unique pour toute la partie (créé à la calibration)
...
$("#btn-calibrate-fake").addEventListener("click", () => {
  ...
  // Le mode test saute la calibration réelle, mais une porte RA (Android +
  // WebXR dispo) a quand même besoin du canvas persistant pour s'ouvrir —
  // sans ça, la première recherche RA plantait (canvas = null).
  if (!state.xrCanvas) {
    const canvas = el("canvas", "xr-canvas xr-hidden");
    canvas.id = "xr-render-canvas";
    document.body.appendChild(canvas);
    state.xrCanvas = canvas;
  }
  beginGameplay();
});
...
$("#btn-calibrate-start").addEventListener("click", async () => {
  ...
  // Canvas créé UNE SEULE FOIS pour toute la partie, jamais retiré du DOM
  // ensuite — la session RA reste ouverte en continu (juste masquée entre
  // deux chapitres) pour que la calibration reste valable jusqu'à la fin.
  ...
  try {
    await ArXR.calibrate({
      ...
      onCalibrated: () => {
        ...
        canvas.classList.add("xr-hidden"); // masqué, pas retiré : la session continue en arrière-plan
        beginGameplay();
      },
      // onEnd ne se déclenche ici que si la RA s'arrête AVANT d'avoir calibré
      // (échec, refus de permission, sortie de l'élève) — pas après un succès.
      onEnd: () => {
        if (!state.calibrated) {
          overlayRoot.classList.add("hidden");
          canvas.remove();
...
function openDossier() {
  ...
  const tabs = ["Pyramide de conversion", "Chiffres romains", "Roue de César"];
...
function routeToGateOrPuzzle(container, ch, fallbackFn, keyOverride) {
  const key = keyOverride || ch.spatialKey;
  const chForGate = keyOverride ? { ...ch, spatialKey: key } : ch;
  ...
  if (needsSpatialGate) {
    renderSpatialGate(container, chForGate, resolvedFallback);
  } else if (needsMinimalArGate) {
    renderMinimalArGate(container, chForGate, resolvedFallback);
  } else {
    resolvedFallback();
  }
}
...
function renderMinimalArGate(container, ch, onFoundFn) {
  container.innerHTML = "";
...
function renderSpatialGate(container, ch, onFoundFn) {
  container.innerHTML = "";
  container.appendChild(el("div", "chapter-text", "<strong>📍 Explorez la salle en réalité augmentée pour trouver l'indice.</strong> Un vieil archiviste attend près de la porte si vous êtes bloqués."));
  ...
  function launch() {
    ...
    $("#xr-exit-btn").textContent = "Quitter la RA immersive"; // évite d'afficher le texte d'un chapitre précédent
    $("#xr-exit-btn").classList.remove("pulse");
    // Canvas et session UNIQUES pour toute la partie (créés une fois à la
    // calibration, réutilisés ici) : plus jamais recréés/retirés du DOM
    // entre deux chapitres, sinon la calibration ("local-floor") repart de
    // l'endroit où on se trouve à cet instant → les objets "bougeaient".
    const canvas = state.xrCanvas;
    canvas.classList.remove("xr-hidden");

    const baseAnchors = (window.SPATIAL_ANCHORS && window.SPATIAL_ANCHORS[ch.spatialKey]) || [];
    ...
    // Sortie contextuelle : que le bouton "Quitter"/"Continuer" fasse la
    // bonne chose selon qu'on a trouvé l'objet ou pas — sans jamais fermer
    // la session RA elle-même (elle reste ouverte pour le chapitre suivant).
    function leaveGate() {
      ArXR.leaveChapter();
      clearTimeout(wrongFeedbackTimer);
      $("#xr-hint-panel").classList.add("hidden");
      overlayRoot.classList.add("hidden");
      canvas.classList.add("xr-hidden");
      if (found) {
        state.score += 3;
        updateScorePill();
        ...
        dommage.textContent = dommage.textContent || "Session RA interrompue — vous pouvez la relancer à tout moment.";
      }
    }
    ...
    ArXR.exploreChapter({
      ...
      onHint: () => {
        ...
        yesBtn.onclick = () => {
          ...
          const hintEl = $(".xr-overlay-hint", overlayRoot);
          clearTimeout(wrongFeedbackTimer);
          hintEl.innerHTML = `💡 ${ch.hint}`;
        };
        noBtn.onclick = close;
      },
      // onEnd ne se déclenche plus qu'en cas de fin RÉELLE de la session RA
      // (permission retirée, navigateur qui coupe) — plus jamais entre deux
      // chapitres normaux, puisqu'on ne referme plus la session à chaque fois.
      onEnd: () => {
        overlayRoot.classList.add("hidden");
        canvas.classList.add("xr-hidden");
        launchBtn.disabled = false;
        launchBtn.textContent = "Réessayer la RA (recalibration nécessaire)";
        dommage.textContent = "La session RA a été coupée par l'appareil. Il faudra recalibrer pour continuer en RA (vous pouvez aussi continuer sans).";
      }
    }).catch((err) => {
      overlayRoot.classList.add("hidden");
      canvas.classList.add("xr-hidden");
  ...
  if (p.type === "jigsaw-image") {
    ...
    function onTileClick(slot) {
      ...
      if (order.every((v, i) => v === i)) {
        ...
        setTimeout(() => {
          container.innerHTML = "";
          container.appendChild(el("div", "chapter-text", "<strong>Bravo, vous avez trouvé Genève !</strong> Avant d'aller plus loin, un dernier repérage : les outils de décodage doivent être quelque part dans la salle — ils vous serviront toute la partie."));
          ...
          // Avant : ce bloc réaffichait "cherchez l'écusson" ici sans jamais
          // lancer la RA correspondante (clé manquante), puis le chapitre 2
          // relançait la même consigne en double juste après — d'où
          // l'impression de décalage entre "trouver Genève" et la suite.
...
function renderAnswerForm(container, ch, getValue) {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (ok) {
      ...
      playTone("right");
      const proceed = () => {
        if (ch.confirmSpatialKey) {
          container.innerHTML = "";
          container.appendChild(el("div", "chapter-text", `<strong>Bonne réponse. Confirmez-le en trouvant, en réalité augmentée, l'objet qui s'y réfère dans la salle.</strong>`));
          routeToGateOrPuzzle(container, ch, () => finishChapter(container, ch), ch.confirmSpatialKey);
        } else {
          finishChapter(container, ch);
        }
      };
