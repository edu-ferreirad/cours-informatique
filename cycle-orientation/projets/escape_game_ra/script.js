...
$all(".mode-card[data-difficulty]").forEach(card => {
  card.addEventListener("click", () => {
    if (card.dataset.difficulty === "bonus") {
      // window.prompt()/alert() sont bloqués ou silencieusement ignorés
      // dans beaucoup de contextes plein écran/PWA installée sur mobile —
      // c'est très probablement pourquoi le mot de passe n'apparaissait
      // jamais. Panneau intégré à la place, comme le reste de l'appli.
      $("#bonus-password-panel").classList.remove("hidden");
      $("#bonus-password-input").value = "";
      $("#bonus-password-error").textContent = "";
      $("#bonus-password-input").focus();
      return;
    }
...
$("#form-bonus-password").addEventListener("submit", (e) => {
  e.preventDefault();
  const val = $("#bonus-password-input").value.trim();
  if (val.toLowerCase() === (window.BONUS_PASSWORD || "").toLowerCase()) {
    $("#bonus-password-panel").classList.add("hidden");
    showScreen("game");
    renderSalleNoireBonus(true);
  } else {
    $("#bonus-password-error").textContent = "Mot de passe incorrect.";
  }
});
$("#bonus-password-cancel").addEventListener("click", () => {
  $("#bonus-password-panel").classList.add("hidden");
});
...
function openDossier() {
  ...
  const tabs = ["Pyramide de conversion", "Chiffres romains", "Roue de César", "Code Morse", "Table des éléments", "Paroles"];
  ...
  function renderTabContent(tab) {
    ...
    if (tab === 5) {
      const lyrics = (window.LA_BELLE_ESCALADE_LYRICS || "").trim();
      const assets = window.GAME_ASSETS || {};
      const pages = [];
      if (assets.img_belle_escalade) pages.push(assets.img_belle_escalade);
      for (let i = 1; i <= 8; i++) { if (assets["cequelaino_page" + i]) pages.push(assets["cequelaino_page" + i]); }
      const gallery = pages.length
        ? `<div style="display:flex; flex-direction:column; gap:10px;">${pages.map(src => `<img src="${src}" style="width:100%; border-radius:6px;">`).join("")}</div>`
        : "";
      if (!lyrics && !gallery) return `<p class="chapter-text" style="text-align:center; opacity:.7;">Les paroles de « La Belle Escalade » n'ont pas encore été ajoutées.</p>`;
      const textBlock = lyrics ? `<div class="chapter-text" style="text-align:left; white-space:pre-line; margin-bottom:12px;">${escapeHtml(lyrics)}</div>` : "";
      return textBlock + gallery;
    }
    return `
...
function renderMinimalArGate(container, ch, onFoundFn, introText) {
  container.innerHTML = "";
  const group = (window.SPATIAL_ANCHORS && window.SPATIAL_ANCHORS[ch.spatialKey]) || [];
  const target = group.find(a => a.correct);
  ...
  const arrow = $(".ar-compass-arrow", block);
  const btnStart = $(".btn-cam-start", block);
  const btnCalibrate = $(".btn-calibrate", block);
  const btnScan = $(".btn-scan", block);

  let headingOffset = null; // cap enregistré au calibrage (= angle 0 du jeu)
  let lastDelta = null;     // écart actuel (degrés) entre la flèche et l'objet le plus proche
  ...
  function startBeeper() {
    ...
    function tick(ts) {
      if (!beeper) return;
      if (ts >= nextBeepAt && lastDelta !== null) {
        const closeness = 1 - Math.min(1, Math.abs(lastDelta) / 180); // 0 (loin) → 1 (aligné)
        const freq = 300 + closeness * 900;
        const gap = 700 - closeness * 600; // ms entre les bips : 700ms loin, 100ms tout proche
        const osc = actx.createOscillator();
        const gain = actx.createGain();
        osc.frequency.value = freq;
        osc.type = "sine";
        gain.gain.setValueAtTime(0.001, actx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.12, actx.currentTime + 0.01);
        gain.gain.exponentialRampToValueAtTime(0.001, actx.currentTime + 0.09);
        osc.connect(gain); gain.connect(actx.destination);
        osc.start(); osc.stop(actx.currentTime + 0.1);
        nextBeepAt = ts + gap;
      }
      beeper.raf = requestAnimationFrame(tick);
    }
  ...
  // cap actuel — c'est lui, pas forcément le bon, qui sera évalué au scan.
  // Sans ça, le mode iOS n'avait AUCUN leurre : on chargeait direct le bon
  // objet, contrairement à Android où les leurres sont vraiment posés dans
  // la pièce et peuvent être tapés par erreur.
  function updateArrow(heading) {
    if (headingOffset === null || !hasAngles) return;
    let best = null, bestDelta = Infinity;
    group.forEach(a => {
      let d = a.angle - (heading - headingOffset);
      d = ((d % 360) + 540) % 360 - 180;
      if (Math.abs(d) < Math.abs(bestDelta)) { bestDelta = d; best = a; }
    });
    aimedAnchor = best;
    lastDelta = bestDelta;
  ...
  async function startCamera() {
    ...
    if (res.ok) {
      ...
      if (hasAngles && ArEngine.headingSupported()) {
        btnCalibrate.classList.remove("hidden");
      } else {
        // Pas de boussole dispo : impossible de distinguer les leurres du
        // bon objet sans direction — on garde le scan simple, jamais
        // bloquant, comme avant (repli sans leurres).
        aimedAnchor = target;
  ...
  btnScan.addEventListener("click", () => {
    ...
    ArEngine.scan({
      ...
      onProgress: (pct) => {
        if (!state.testMode) status.textContent = `Analyse en cours… (${Math.round(pct * 100)}%)`;
      },
      onDone: () => {
        const hit = aimedAnchor;
        if (hit && hit.correct) {
          ArEngine.unwatchHeading();
          status.textContent = "Indice trouvé ✓";
          status.className = "ar-status ok";
          arrow.classList.add("hidden");
          clueOverlay.textContent = `✨ Trouvé ! « ${label} »`;
          ...
          state.lastGateWrongCount += 1;
          playTone("wrong");
          status.textContent = `✗ Ce n'est pas « ${hit.label} ».`;
          status.className = "ar-status";
          clueOverlay.textContent = hit.romanTrap ? "IV · XV · XIII · XIII · I · VII · V" : "Continuez à chercher…";
          clueOverlay.classList.add("visible");
          setTimeout(() => clueOverlay.classList.remove("visible"), 1800);
          btnScan.disabled = false;
          startBeeper();
        } else {
          status.textContent = "Rien de précis par ici — visez mieux et réessayez.";
          btnScan.disabled = false;
          startBeeper();
        }
      }
...
function renderPuzzle(container, ch) {
  ...
  if (p.type === "jigsaw-image") {
    ...
    // La boîte ouverte ne suffit pas : il faut encore trouver, en RA dans
    // la salle, le vieux panneau où est punaisé "Cé qu'è lainô" — c'est en
    // le zoomant que la vraie photo de la strophe 14 apparaît.
    if (p.songSpatialKey && !state.foundSong) {
      routeToGateOrPuzzle(container, ch, () => {
        state.foundSong = true;
        renderPuzzle(container, ch);
      }, p.songSpatialKey, "<strong>La boîte est ouverte. Cherchez maintenant, en réalité augmentée, le vieux panneau où est punaisée une chanson qui parle d'Isaac Mercier.</strong>");
      return;
    }
    const widget = el("div", "puzzle-widget");
    widget.style.cssText = "background:#f2ead6; border-radius:10px; padding:16px; text-align:center; color:#2a2318;";
    const photoUrl = (window.GAME_ASSETS && window.GAME_ASSETS.img_cequelaino_strophe14) || "";
    widget.innerHTML = `
      <div style="font-family:Georgia, serif; font-size:13px; letter-spacing:2px; text-transform:uppercase; opacity:.7;">Un vieux feuillet</div>
      <div style="font-family:Georgia, serif; font-style:italic; font-size:16px; margin:6px 0 14px;">« Cé qu'è lainô »</div>
      ${photoUrl ? `<img src="${photoUrl}" alt="Feuillet — strophe 14" style="max-width:100%; border-radius:6px;">` : `<div style="font-family:var(--font-mono); font-size:34px; letter-spacing:14px;">${p.expression}</div>`}
    `;
    container.appendChild(widget);
    container.appendChild(el("div", "chapter-text", p.prompt));
    renderAnswerForm(container, ch);
...
function renderSalleNoireSenses(standalone) {
  ...
  card.innerHTML = `
    ...
    <button class="btn-primary hidden" id="sn-explore-btn">🔦 Explorer la salle en RA</button>
  ...
  const exploreBtn = $("#sn-explore-btn", card);
  ...
  // ---------------- Icônes des 5 sens : trouvées en RA dans le noir ----------------
  const SENSE_ICON_BOXES = { vue: vueBox, toucher: touchBox, ouie: earBox, odorat: smellBox, gout: tasteBox };
  const SENSE_ANCHOR_KEY = { sens_vue: "vue", sens_toucher: "toucher", sens_ouie: "ouie", sens_odorat: "odorat", sens_gout: "gout" };
  function revealSense(key) {
    const box = SENSE_ICON_BOXES[key];
    if (box) box.classList.remove("hidden");
  }
  // Repli (appareils sans RA immersive) : grille plate, comme avant.
  ...
  $("#sn-dark-start", card).addEventListener("click", async () => {
    ...
    if (!res.ok) {
      ...
      if (!$("#sn-dark-fallback", card)) {
        ...
        fb.addEventListener("click", () => {
          darkStatus.textContent = "Obscurité confirmée ✓";
    ...
    if (state.testMode) {
      darkStatus.textContent = "Mode test ✓";
      unlockExploration();
    }
    ArEngine.watchDarkness(isDark => {
      if (isDark) {
        darkStatus.textContent = "Obscurité détectée ✓";
        unlockExploration();
        playTone("right");
      } else {
  ...
  exploreBtn.addEventListener("click", async () => {
    ...
    // Bug trouvé : la caméra de la lampe UV (ArEngine, un flux getUserMedia
    // séparé) restait ouverte, ce qui empêchait la session WebXR immersive
    // de démarrer correctement (conflit d'accès à la caméra) — d'où le
    // calibrage qui ne réagissait jamais au tap. On la libère avant.
    ArEngine.detach();
    const webxrOk = state.mode === "android" && state.webxrAvailable && ArXR.isAvailable && await ArXR.isAvailable();
    if (!webxrOk) {
      // Repli : pas de RA immersive disponible sur cet appareil — grille plate.
      exploreBtn.classList.add("hidden");
      iconRow.classList.remove("hidden");
      return;
    }
    const anchors = (window.SPATIAL_ANCHORS && window.SPATIAL_ANCHORS.salle_noire) || [];
    const overlayRoot = $("#xr-overlay-root");
    overlayRoot.classList.remove("hidden");
    $(".xr-overlay-hint", overlayRoot).textContent = window.CALIBRATION_POINT_DESC
      ? `D'abord, ${window.CALIBRATION_POINT_DESC} puis touchez l'écran pour calibrer.`
      : "Touchez l'écran pour calibrer ici.";
    $("#xr-exit-btn").textContent = "Quitter la RA immersive";
    $("#xr-exit-btn").classList.remove("pulse");
    const canvas = el("canvas", "xr-canvas");
    canvas.id = "xr-render-canvas";
    ...
    try {
      await ArXR.exploreChapter({
        ...
        multiFind: true,
        onCalibrated: () => {
          // Bug corrigé : le message restait sur "touchez l'écran pour
          // calibrer" même une fois le calibrage réussi, donc on ne
          // savait jamais si ça avait marché.
          $(".xr-overlay-hint", overlayRoot).textContent = "Calibrage fait ✓ — cherchez les 5 sens dans la salle et touchez-les.";
        },
        onFound: (anchor) => {
          const key = SENSE_ANCHOR_KEY[anchor.id];
          if (key) revealSense(key);
          playTone("right");
        },
        onEnd: () => {
          overlayRoot.classList.add("hidden");
      ...
      canvas.remove();
      exploreBtn.disabled = false;
      // Repli si la session RA échoue en cours de route.
      iconRow.classList.remove("hidden");
    }
  });
...
function buildQcm10(container, options, correctIndex) {
  ...
  options.forEach((label, i) => {
    const btn = el("button", "mode-card", `<div class="mode-card-desc" style="font-size:12px; display:flex; align-items:center; gap:6px;"><strong style="color:var(--flame-bright); font-family:var(--font-mono); min-width:14px;">${i}</strong> ${label}</div>`);
    btn.type = "button";
...
function renderEndCredits() {
  ...
  credits.innerHTML = `
    <div class="end-credits-scroll">
      ...
      <div>—</div>
      <div>Créé par David Ferreira</div>
      <div>Corrigé et optimisé par Claude AI</div>
      <div>—</div>
      ${songBlock}
    </div>
  `;
